      ...
      else
      {
        ...
        if ($user->hasRole('penjaga') || $user->hasRole('pemilik')) {
          ...
          if ($request->has('simpanpenjualan')) {             
            ...
            $kas->kredit = $penjualans->total_harga_jual;
            $kas->createdby = $penjualans->createdby;
            $list_kas = Kas::all();  
            $total_saldo_harian = 0;
            $cek_tgl_kas = Kas::where('tanggal', $kas->tanggal)->first();            
            if (!is_null($cek_tgl_kas)) {
              if ($kas->tanggal == $cek_tgl_kas->tanggal) {
                foreach ($list_kas as $kas_listed) {
                  if ($kas->tanggal == $kas_listed->tanggal) {
                    $total_saldo_harian += $kas_listed->debit;
                  }        
                }
                $kas->saldo = $kas->debit + $total_saldo_harian;              
                $kas ->save();                
              }              
            } else {
              $kas->saldo = $kas->debit;
